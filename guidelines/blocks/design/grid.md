...
- **Spacing:** Define row and column gaps using spacing tokens such as `--wp--preset--spacing--sm` and `--wp--preset--spacing--md`. Avoid adding individual margins to grid items.
...
<Grid columns={3} minWidth="250px" rowGap="var(--wp--preset--spacing--md)" columnGap="var(--wp--preset--spacing--md)">
...
| `minWidth`   | string  | `200px` | Minimum width for each grid item; used when `columns` is undefined to calculate auto‑filling columns. |
...
| `sticky`     | boolean | false   | Whether the grid is sticky (sticks to top of viewport). |
...
- **Columns block:** For simple column layouts【425069070290782†L87-L120】.
- **Row block:** For horizontal alignment【421014285023784†L73-L168】.
